...
<body class="editar">
  <?php
  //incluyo BD ,usuarioes ya que voy a crear uno nuevo y los metodos para poder utilizarlos
  include '../database/boots/header.php';
  require_once '../Surface/database/fileHandler/registroContext.php';
   ...
   require_once '../Surface/logica/usuario.php';
  ...
  if (isset($_GET['id'])) {
    ...
    if (isset($_POST["nombre"]) && isset($_POST["region"]) && isset($_POST["tipo"])) { //Verificamos que todos los elementos esten
      ...
      $usuario = new usuario(); // creamos un nuevo objeto o usuario 

      $usuario->InicializeData($pokeId, $_POST["nombre"], $_FILES["pokeFoto"], $_POST["region"], $_POST["tipo"], $_POST["Stipo"], $_POST["color"]); //seteamos los valores

      $service->Update($pokeId, $usuario); //Lo actualizamos con el metodo previamente hecho update

      header("location:/pokedex/pokeList.php"); //redireccionamos la pagina a la lista de usuarioes
    }
  }
  ...
  <div>
    ...
    <div class="formulario-editar">
      ...
      <!--Get Post redirect-->
      <!-- especificamo el usuario que se va a editar atraves del id que tiene -->
